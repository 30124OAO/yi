...
-- Delta安卓注入器文件路径
local SAVE_PATH = "/storage/emulated/0/Delta/Autoexecute/chicken_timer.txt"
local hangupSecond = 0

-- Delta注入器API安全封装
local function TryReadTimerFile()
    if not _G.delta or not _G.delta.files then
        return 0
    end
    local ok, content = pcall(function()
        return _G.delta.files.read(SAVE_PATH)
    end)
    ...
    --读取完毕删除文件
    pcall(function()
        _G.delta.files.remove(SAVE_PATH)
    end)
    return num
end

local function TrySaveTimerFile(totalSec)
    if not _G.delta or not _G.delta.files then
        return
    end
    pcall(function()
        _G.delta.files.write(SAVE_PATH, tostring(totalSec))
    end)
end

--启动读取存档
hangupSecond = TryReadTimerFile()

--秒转 HH:MM:SS
local function SecondToHMS(totalSec)
...
MainPanel.Size = UDim2.new(0, 380, 0, 390)
MainPanel.Position = UDim2.new(0.5, -190, 0.5, -195)
...
VersionTip.Text = "更新时间:8月29日13时45分，更新内容:修复了自动捡垃圾因为离鸡舍过远导致无法重生情况、更新可滑动进度条"
...
local HangupTimeLabel = Instance.new("TextLabel")
HangupTimeLabel.Size = UDim2.new(0.94,0,0,22)
HangupTimeLabel.Position = UDim2.new(0.03,0,0.81,0)
HangupTimeLabel.BackgroundTransparency = 1
HangupTimeLabel.TextColor3 = Color3.new(0.2,1,0.35)
HangupTimeLabel.Font = Enum.Font.SourceSansBold
HangupTimeLabel.TextSize = 11
HangupTimeLabel.TextWrapped = true
HangupTimeLabel.Parent = MainPanel
...
table.insert(threadPool, task.spawn(function()
    while task.wait(1) do
        hangupSecond +=1
        HangupTimeLabel.Text = string.format("您使用此脚本已挂机 %s", SecondToHMS(hangupSecond))
    end
end))
