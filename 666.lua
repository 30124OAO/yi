...
VersionTip.Text = "更新:TOWER触发改成TowerElevator+TowerStart，实时读取Tower数值"
...
local function ShowTowerCountdownNotify(sec)
    ...
    TowerCountdownNotify.Text = string.format("即将在%d秒后挑战塔",sec)
    task.spawn(function()
        task.wait(sec)
        TowerCountdownNotify.Visible = false
    end)
        ...
        while loopChallengeRunning and globalRunning do
            ...
            if foundTower and towerKeywordCooldown <= 0 then
                ...
                table.insert(threadPool, task.spawn(function()
                    ...
                    task.wait(delaySec)
                    if not (loopChallengeRunning and globalRunning) or suspendTowerLoops then return end
                    pcall(function()
                        --实时读取本地玩家leaderstats.Tower的数值
