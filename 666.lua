...
-- ★新增标记：当前是否已经存在一轮正在进行的倒计时（一次性模式）
local towerCountdownActive = false
...
VersionTip.Text = "塔计时｜一次性倒计时，TOWER消失立刻停止计时，跑完不会自动轮回"
...
local function startLoop()
    ...
    if not towerTimerRunning then
        ...
        towerCountdownActive = false
        table.insert(threadPool, task.spawn(function()
            while loopChallengeRunning and globalRunning do
                task.wait(1)
                -- 主循环这里只做倒计时扣时；TOWER检测交给UI扫描循环
                if towerCountdownActive and towerRemainingTime > 0 then
                    towerRemainingTime -= 1
                    -- 时间走完，执行挑战塔（一次性模式，跑完关闭倒计时状态）
    ...
    table.insert(threadPool, task.spawn(function()
        ...
        while loopChallengeRunning and globalRunning do
            ...
            if not foundTower then
                --TOWER文字消失 → 立刻关闭弹窗、停止当前倒计时
                ClearTowerPopup()
                towerCountdownActive = false
                towerRemainingTime = 0
            else
                --TOWER文字存在
                -- 当前**没有正在进行的倒计时** → 开启新一轮倒计时
                if not towerCountdownActive and towerKeywordCooldown <=0 then
                    towerRemainingTime = towerDelaySliderValue
                    towerCountdownActive = true
                    towerKeywordCooldown = towerDelaySliderValue
                end
                -- 更新弹窗（弹窗不存在就新建；存在就更新文字）
                if towerCountdownActive then
