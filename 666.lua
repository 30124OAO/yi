...
--重生提示弹窗（右上角）
local RebornNotify = Instance.new("TextLabel")
RebornNotify.Size = UDim2.new(0,240,0,44)
RebornNotify.Position = UDim2.new(1,-250,0,10)
RebornNotify.BackgroundColor3 = Color3.new(0.15,0.45,0.75)
RebornNotify.BackgroundTransparency = 0
RebornNotify.BorderColor3 = Color3.new(1,1,1)
RebornNotify.BorderSizePixel = 1
RebornNotify.Text = "已检测到可重生，正在返回"
RebornNotify.TextColor3 = Color3.new(1,1,1)
RebornNotify.Font = Enum.Font.SourceSansBold
RebornNotify.TextSize = 14
RebornNotify.Visible = false
RebornNotify.Parent = ScreenGui

--TOWER倒计时弹窗
local TowerCountdownNotify = Instance.new("TextLabel")
TowerCountdownNotify.Size = UDim2.new(0,240,0,44)
TowerCountdownNotify.Position = UDim2.new(1,-250,0,60)
TowerCountdownNotify.BackgroundColor3 = Color3.new(0.55,0.35,0.1)
TowerCountdownNotify.BackgroundTransparency = 0
TowerCountdownNotify.BorderColor3 = Color3.new(1,1,1)
TowerCountdownNotify.BorderSizePixel = 1
TowerCountdownNotify.Text = ""
TowerCountdownNotify.TextColor3 = Color3.new(1,1,1)
TowerCountdownNotify.Font = Enum.Font.SourceSansBold
TowerCountdownNotify.TextSize = 14
TowerCountdownNotify.Visible = false
TowerCountdownNotify.Parent = ScreenGui

--正在挑战塔弹窗
local TowerRunningNotify = Instance.new("TextLabel")
TowerRunningNotify.Size = UDim2.new(0,240,0,44)
TowerRunningNotify.Position = UDim2.new(1,-250,0,110)
TowerRunningNotify.BackgroundColor3 = Color3.new(0.2,0.6,0.25)
TowerRunningNotify.BackgroundTransparency = 0
TowerRunningNotify.BorderColor3 = Color3.new(1,1,1)
TowerRunningNotify.BorderSizePixel = 1
TowerRunningNotify.Text = "正在挑战塔"
TowerRunningNotify.TextColor3 = Color3.new(1,1,1)
TowerRunningNotify.Font = Enum.Font.SourceSansBold
TowerRunningNotify.TextSize = 14
TowerRunningNotify.Visible = false
TowerRunningNotify.Parent = ScreenGui
...
MainPanel.Size = UDim2.new(0, 380, 0, 360)
MainPanel.Position = UDim2.new(0.5, -190, 0.5, -180)
...
VersionTip.Text = "更新时间:9月6日14时16分，更新内容:修复了部分bug和更新可修改挑战时间"
...
toggleBtn.Position = UDim2.new(0.03, 0, 0.23, 0)
...
DelaySliderFrame.Position = UDim2.new(0.03,0,0.32,0)
...
afkMoveBtn.Position = UDim2.new(0.03, 0, 0.41, 0)
...
scrapBtn.Position = UDim2.new(0.03, 0, 0.50, 0)
...
SliderFrame.Position = UDim2.new(0.03,0,0.59,0)
...
TipText.Position = UDim2.new(0.03,0,0.65,0)
...
NoticeText.Position = UDim2.new(0.03,0,0.71,0)
...
--右上角重生弹窗显示+6.66秒淡出
local function ShowRebornNotify()
    RebornNotify.Visible = true
    RebornNotify.BackgroundTransparency = 0
    RebornNotify.TextTransparency = 0
    RebornNotify.Text = "已检测到可重生，正在返回"
    task.spawn(function()
        task.wait(6.66)
        for i=1,33 do
            RebornNotify.BackgroundTransparency += 0.03
            RebornNotify.TextTransparency += 0.03
            task.wait(0.2)
        end
        RebornNotify.Visible = false
    end)
...
--TOWER倒计时提示，支持每秒刷新，并可被外部提前关闭
local countdownRunning = false
local countdownCancel = false

local function ShowTowerCountdownNotify(sec)
    TowerCountdownNotify.Visible = true
    TowerCountdownNotify.BackgroundTransparency = 0
    TowerCountdownNotify.TextTransparency = 0
    countdownRunning = true
    countdownCancel = false

    task.spawn(function()
        local remaining = sec
        while remaining > 0 and countdownRunning and not countdownCancel do
            TowerCountdownNotify.Text = string.format("即将在%d秒后挑战塔", remaining)
            task.wait(1)
            remaining -= 1
        end

        if not countdownCancel then
            TowerCountdownNotify.Text = "即将在0秒后挑战塔"
        end
...
local function CancelTowerCountdownNotify()
...
--正在挑战塔提示（固定3.8秒）
local function ShowTowerRunningNotify()
    TowerRunningNotify.Visible = true
    TowerRunningNotify.BackgroundTransparency = 0
    TowerRunningNotify.TextTransparency = 0
    TowerRunningNotify.Text = "正在挑战塔"
    task.spawn(function()
        task.wait(3.8)
        TowerRunningNotify.Visible = false
...
local function startLoop()
    ...
    table.insert(threadPool, task.spawn(function()
        local keywordRebirth = "Reborn"
        local keywordTower = "TOWER"
        local towerKeywordCooldown = 0
        local towerTriggered = false
        ...
        while loopChallengeRunning and globalRunning do
            ...
            --TOWER文字消失 → 终止整套塔式序列（倒计时、电梯、进塔全部停止，状态重置）
            if not foundTower then
                CancelTowerCountdownNotify()
            ...
            --TOWER文字出现才执行进塔
            if foundTower and towerKeywordCooldown <= 0 and not towerTriggered then
                local delaySec = towerDelaySliderValue
                towerKeywordCooldown = delaySec
                towerTriggered = true

                table.insert(threadPool, task.spawn(function()
                    ShowTowerCountdownNotify(delaySec)

                    for i = 1, delaySec do
                        --每一秒循环都校验TOWER状态，消失立刻终止全部塔流程
                        if not foundTower or not loopChallengeRunning or globalRunning == false or suspendTowerLoops then
                            CancelTowerCountdownNotify()
                            towerTriggered = false
                            return
                        end
                    ...
                    pcall(function()
                        local towerValue = localPlayer.leaderstats.Tower.Value
                        local args = {[1] = towerValue}
