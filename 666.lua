...
local function CreatePopup(text, lifeTime)
	...
	task.spawn(function()
		local fadeStart = lifeTime - 1.2 -- 最后1.2秒执行淡出
		task.wait(fadeStart)
		-- 淡出动画
		for i = 1, 24 do
			local alpha = i / 24
			PopFrame.BackgroundTransparency = alpha
			PopFrame.BorderTransparency = alpha
			PopLabel.TextTransparency = alpha
			task.wait(0.05)
		end
		task.wait(lifeTime - fadeStart)
...
local function startLoop()
	...
	table.insert(threadPool, task.spawn(function()
		...
		while loopChallengeRunning and globalRunning do
			...
			--重生检测+右上角弹窗（通知时长6.66秒，带淡出）
			if foundRebirth and triggerCooldown <= 0 then
				CreatePopup("已检测到可重生，正在返回",6.66)
...
print("✅ 修改版脚本加载成功｜Rebirth间隔0.1s、更新注释、移除TOWER弹窗、重生通知6.66秒+淡出动画")
ConfirmPanel:ToFront()
