...
-- 读取本地存储的挂机时间，读完清空存储
if PlayerPrefs:HasKey("ChickenHangupSec") then
    hangupSecond = PlayerPrefs:GetInt("ChickenHangupSec")
    PlayerPrefs:RemoveKey("ChickenHangupSec")
end
...
-- 退出/关闭面板：保存挂机秒数到本地存储
local function SaveHangupTime()
    PlayerPrefs:SetInt("ChickenHangupSec", hangupSecond)
    PlayerPrefs:Flush()
end
