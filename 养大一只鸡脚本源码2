...
local rebirthInterruptScrap = false
local hiddenScrapCounter = 0
...
local function IsOnScreen(guiObj)
    ...
    local x2, y2 = absPos.X + absSize.X, absSize.Y
...
local function ScrapMainLoop()
    local keywordRebirthScrap = "REBIRTH READY!"
    hiddenScrapCounter = 0
    while autoScrapRunning and globalRunning do
        local delayTime = 0.5
        local interruptRebirthFound = false
        local playerGui = localPlayer:FindFirstChild("PlayerGui")
        if playerGui then
            for _,child in ipairs(playerGui:GetDescendants()) do
                if CheckVisibleText(child) and string.find(child.Text,keywordRebirthScrap,1,true) then
                    interruptRebirthFound = true
                    break
                end
            end
        end
        if interruptRebirthFound and recycleDestination then
            rebirthInterruptScrap = true
        ...
        local recyclerUI = workspace:FindFirstChild("Recyclers") and workspace.Recyclers:FindFirstChildOfClass("Part")
        if recyclerUI then
            recycleDestination = recyclerUI.Position
        end
        local Character = localPlayer.Character
        ...
        local state
        if hiddenScrapCounter < selectNum then
            state = "FindScrap"
        else
            state = "GoHome"
        end
            ...
            for _, item in ipairs(workspace:GetChildren()) do
                if item:IsA("Model") and item.Name == "PitScrap" then
                    ...
                    if scrapPart then
                        ...
                        table.insert(scrapTable, {Model = item,PartPos = scrapPart.Position,Dist = dist})
                    end
                end
            end
